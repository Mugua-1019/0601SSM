<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ page import="hospital.model.Medicine" %>
<%@ page import="java.util.List" %>
<%
    List<Medicine> medicines = (List<Medicine>) request.getAttribute("medicines");
%>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title>药品管理</title>
    <link href="css/style.css" rel="stylesheet" type="text/css" />
    <script type="text/javascript" src="js/jquery.min.js"></script>
    <script type="text/javascript">$(function(){ $('tbody tr:odd').addClass("trLight"); });</script>
    <style type="text/css">body { background:#FFF }</style>
</head>
<body>
<div id="contentWrap">
    <div id="widget table-widget">
        <div class="pageTitle">药品管理</div>
        <div class="querybody">
            <ul class="seachform">
                <li><label>药品名称</label><input name="name" type="text" class="scinput" /></li>
                <li><label>&nbsp;</label><input type="button" class="scbtn" value="查询"/></li>
            </ul>
        </div>
        <div class="pageColumn">
            <div class="pageButton"><a href="medicines?action=new"><img src="images/t01.png" title="新增"/></a><span>药品列表</span></div>
            <table>
                <thead>
                <th width="">药品ID</th>
                <th width="">药品名</th>
                <th width="">规格</th>
                <th width="">库存数量</th>
                <th width="">售价</th>
                <th width="10%">操作</th>
                </thead>
                <tbody>
                <% if (medicines == null || medicines.isEmpty()) { %>
                <tr><td colspan="6">暂无药品数据</td></tr>
                <% } else { for (Medicine medicine : medicines) { %>
                <tr>
                    <td><%= medicine.getId() %></td>
                    <td><%= medicine.getName() %></td>
                    <td><%= medicine.getSpecification() == null ? "" : medicine.getSpecification() %></td>
                    <td><%= medicine.getStock() %></td>
                    <td><%= medicine.getPrice() %></td>
                    <td>
                        <a href="medicines?action=edit&id=<%= medicine.getId() %>"><img src="images/icon/edit.png" width="16" height="16" /></a>
                        <a href="medicines?action=delete&id=<%= medicine.getId() %>" onclick="return confirm('确认删除该药品吗？');"><img src="images/icon/del.png" width="16" height="16" /></a>
                    </td>
                </tr>
                <% }} %>
                </tbody>
            </table>
        </div>
    </div>
</div>
</body>
</html>
